...
# ---------- 路径配置（基于脚本位置自动定位）----------
# 脚本位于 dataset/scripts/paste_aug.py
# 则 BASE_DIR = dataset/
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# 数据根目录为 dataset/paste_red
DATA_DIR = os.path.join(BASE_DIR, "paste_red")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(BG_DIR):
        print(f"❌ 错误：背景文件夹不存在，请创建：{BG_DIR}")
        exit(1)
    if not os.path.exists(OBJ_DIR):
        print(f"❌ 错误：物体文件夹不存在，请创建：{OBJ_DIR}")
        exit(1)
